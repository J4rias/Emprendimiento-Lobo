...
const skuConfig = {
   ...
   */
  getBrandCode(brandName: string): string {
   ...
   */
  getShortName(productName: string): string {
    ...
    const abbreviated = significantParts.map((word: string) => {
   ...
   */
  getProductInfo(unit_size: any, unit_size_measure: any): { content: string; uom: string } {
    let content: string = unit_size || '1';
    const uom: string = unit_size_measure || 'UND';
   ...
   */
  generateHash(brand_id: any, productName: string, content: string, uom: string, existingSku: string | null = null): string {
   ...
   */
  removeRedundantBrand(shortName: string, brandCode: string): string {
    if (!shortName || !brandCode) return shortName;

    const parts = shortName.split('-');
    // Eliminar cualquier parte que coincida con el código de marca
    const filtered = parts.filter((part: string) => part !== brandCode);

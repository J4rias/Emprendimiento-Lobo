...
describe('Brands API — smoke tests', () => {
  it('GET /api/brands sin auth -> 401 o 403', async () => {
    const res = await request(app).get('/api/brands');
    expect(res.status).toBeGreaterThanOrEqual(401);
    expect(res.status).toBeLessThan(500);
  });

  it('GET /api/brands con token -> 200, body.data es array', async () => {
    const res = await request(app)
      .get('/api/brands')
  ...
  it('POST /api/brands con body vacío {} CON token -> 400 o 422', async () => {
    const res = await request(app)
      .post('/api/brands')
      .set('Authorization', `Bearer ${authToken}`)
      .send({});

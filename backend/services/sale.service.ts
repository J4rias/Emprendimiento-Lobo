  ...
  try {
    ...
    for (const item of items) {
      ...
      inventoryMovements.push({
        ...
        package_quantity: is_unit ? 0 : item.quantity,
        loose_units: is_unit ? item.quantity : 0,
      ...
      if (inventory) {
        ...
        if (['completed', 'pending', 'partial'].includes(sale.status)) {
          ...
          cancelMovements.push({
            ...
            package_quantity: detail.is_unit ? 0 : parseFloat(detail.quantity),
            loose_units: detail.is_unit ? parseFloat(detail.quantity) : 0,

...
// ─── getStatement (legacy endpoint) ──────────────────────────────────────────

export async function getSupplierStatement(supplierId: string | number) {
  ...
  for (const po of purchaseOrders) {
    ...
    const rateToUSD = await (ExchangeRate as any).getRate(poCurrency, 'USD', po.order_date);
    const rateToCOP = await (ExchangeRate as any).getRate(poCurrency, 'COP', po.order_date);
    ...
    ledger.push({
      id: `po_${po.id}_usd`, type: 'charge', date: new Date(po.order_date),
      reference: po.order_number, amount: amtUSD, currency: 'USD',
      description: `Orden de Compra: ${po.status}`, original_amount: amountOrig,
      original_currency: poCurrency, original_data: po
    });

    if (!summary['COP']) summary['COP'] = { total_invoiced: 0, total_paid: 0, balance: 0, available_credit: 0 };
    summary['COP'].total_invoiced += amtCOP;
    ledger.push({
      id: `po_${po.id}_cop`, type: 'charge', date: new Date(po.order_date),
      reference: po.order_number, amount: amtCOP, currency: 'COP',
      description: `Orden de Compra: ${po.status}`, original_amount: amountOrig,
      original_currency: poCurrency, original_data: po
    });
  }
  ...
  for (const pay of payments) {
    ...
    const rateToUSD = await (ExchangeRate as any).getRate(payCurrency, 'USD', pay.payment_date);
    const rateToCOP = await (ExchangeRate as any).getRate(payCurrency, 'COP', pay.payment_date);
    const amtUSD = amountOrig * rateToUSD;
    const amtCOP = amountOrig * rateToCOP;
    ...
    ledger.push({
      id: `pay_${pay.id}_usd`, type: 'payment', date: new Date(pay.payment_date),
      reference: pay.payment_number, amount: amtUSD, currency: 'USD',
      description: `Abono (${pay.payment_method})`, original_amount: amountOrig,
      original_currency: payCurrency, original_data: pay
    });

    if (!summary['COP']) summary['COP'] = { total_invoiced: 0, total_paid: 0, balance: 0, available_credit: 0 };
    summary['COP'].total_paid += amtCOP;
    ledger.push({
      id: `pay_${pay.id}_cop`, type: 'payment', date: new Date(pay.payment_date),
      reference: pay.payment_number, amount: amtCOP, currency: 'COP',
      description: `Abono (${pay.payment_method})`, original_amount: amountOrig,
      original_currency: payCurrency, original_data: pay
    });
  }

 ...
 */
function computeDueDate(sale: any, creditDays: any) {
 ...
 */
function computeAgingBucket(dueDate: any) {
  if (!dueDate) return { bucket: 'sin_termino', daysOverdue: 0 };
  const now = new Date();
  const diffMs = now.getTime() - (dueDate as Date).getTime();
 ...
 */
async function buildCustomerStatement(customerId: any) {
  ...
  const canReverseMap: Record<string, any> = {};
  ...
  const realPaymentsBySale: Record<string, any> = {};
  for (const pay of payments) {
  ...
  for (const sale of sales) {
    ...
    const paidCOP = realPays.reduce((s: number, p: any) => {
  ...
  return {
    customer: {
      id: customer.id,
      code: customer.code,
      name: customer.getFullName ? customer.getFullName() : `${(customer as any).first_name} ${(customer as any).last_name}`,
      documentNumber: (customer as any).document_number,
      creditDays: (customer as any).credit_days || 0
    },
    summary,
    ledger
  };
...
async function getCustomerCreditBlock(customerId: any) {
  ...
  const pendingSales = allSales.filter((s: any) =>
  ...
  for (const sale of pendingSales) {
    ...
    if (dueDate && dueDate < now) {
      ...
      return {
        blocked: true,
        reason: `${(sale as any).sale_number} vencida hace ${daysOverdue} día${daysOverdue !== 1 ? 's' : ''}`,
...
export = { buildCustomerStatement, getCustomerCreditBlock, computeDueDate, computeAgingBucket };

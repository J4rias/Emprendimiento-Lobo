...
export const reserve = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    const presentation = await ProductPresentation.findByPk(presentation_id, { transaction }) as any;
    if (!presentation) {
      await transaction.rollback();
      return res.status(404).json({
    ...
    const available = parseFloat(inventory.quantity) - (reservedByOthers || 0);
    ...
    if (available < units_requested) {
      ...
      return res.status(409).json({
        ...
        reserved_by_others: reservedByOthers || 0
      });
    }
 ...
 */
export const releaseItem = async (req: Request, res: Response, next: NextFunction) => {
  const transaction = await sequelize.transaction();
 ...
 * DELETE /api/pos/tab
 */
export const releaseTab = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const {
      session_id,
      tab_id
 ...
 */
export const cleanupExpired = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const now = new Date();

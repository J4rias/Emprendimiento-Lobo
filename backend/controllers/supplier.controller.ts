  ...
  try {
    ...
    for (const pay of payments) {
      ...
      if (pay.exchange_rate && pay.exchange_rate_from && pay.exchange_rate_to) {
        ...
        rateToUSD = await (ExchangeRate as any).getRate(payCurrency, 'USD', pay.payment_date);
        rateToCOP = await (ExchangeRate as any).getRate(payCurrency, 'COP', pay.payment_date);
      } else {
        rateToUSD = await (ExchangeRate as any).getRate(payCurrency, 'USD', pay.payment_date);
        rateToCOP = await (ExchangeRate as any).getRate(payCurrency, 'COP', pay.payment_date);
      }
...
function getLedgerCategory(currency: string, settlementCurrency: string): string {
...
function inferCategoryFromPaymentCurrency(paymentCurrency: string): string {
...
export const getLedger = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    for (const cat in categories) {
      ...
      categories[cat].invoices.sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
      categories[cat].payments.sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
    }

    // 6. Get current BCV rate
    let bcvRate = null;
    try {
      bcvRate = await (ExchangeRate as any).getRate('USD', 'VES');

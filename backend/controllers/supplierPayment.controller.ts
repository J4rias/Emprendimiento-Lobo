...
export const createPayment = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const supplier = await Supplier.findByPk(supplier_id) as any;
    if (!supplier) {
      await transaction.rollback();
      return res.status(404).json({
    ...
    if (allocationsList.length > 0) {
      ...
      for (const alloc of allocationsList) {
        const po = await PurchaseOrder.findByPk(alloc.purchase_order_id) as any;
        if (!po) {
          await transaction.rollback();
        ...
        if (currency === poCurrency) {
          ...
          if (exchange_rate_from === currency && exchange_rate_to === poCurrency) {
            ...
            try {
              allocAmountInPOCurrency = await (ExchangeRate as any).convert(parseFloat(alloc.allocated_amount), currency, poCurrency, new Date(payment_date));
            } catch (e) {
              allocAmountInPOCurrency = parseFloat(alloc.allocated_amount);
            }
          }
      ...
      const po = await PurchaseOrder.findByPk(purchase_order_id) as any;
      if (!po) {
        await transaction.rollback();
      ...
      if (currency === poCurrency) {
        ...
        if (exchange_rate_from === currency && exchange_rate_to === poCurrency) {
          ...
          try { frozenAmount = await (ExchangeRate as any).convert(parseFloat(amount), currency, poCurrency, new Date(payment_date)); } catch (e) { frozenAmount = parseFloat(amount); }
        }
...
export const updatePayment = async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Find payment
    const payment = await SupplierPayment.findByPk(id) as any;
    if (!payment) {
      await transaction.rollback();
      return res.status(404).json({
        message: 'Pago no encontrado'
...
export const deletePayment = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const payment = await SupplierPayment.findByPk(id, { transaction }) as any;
    if (!payment) {
      await transaction.rollback();
 ...
 */
export const getPaymentsBySupplier = async (req: Request, res: Response) => {
  try {

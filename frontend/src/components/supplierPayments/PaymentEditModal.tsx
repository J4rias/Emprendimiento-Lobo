...
interface PaymentEditModalProps {
  ...
  open: boolean;
  onClose: () => void;
  onSubmit: (form: PaymentEditForm) => void;
  isPending: boolean;
}
...
export function PaymentEditModal({ payment, open, onClose, onSubmit, isPending }: PaymentEditModalProps): React.ReactElement {
  const [form, setForm] = useState<PaymentEditForm>(DEFAULT_FORM);
  ...
  const set = (field: keyof PaymentEditForm) => (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => setForm((prev) => ({ ...prev, [field]: e.target.value }));

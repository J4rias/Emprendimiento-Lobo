...
const customerName = (c) => {
  if (!c) return '—';
  if (c.business_name || c.businessName) return c.business_name || c.businessName;
  return `${c.first_name || c.firstName || ''} ${c.last_name || c.lastName || ''}`.trim() || '—';
};

const QuoteViewSheet = ({ open, onClose, quote, hasPermission, onStatusChange, onConvert, statusMutation }) => {

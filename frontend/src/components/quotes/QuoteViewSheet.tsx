...
interface QuoteViewSheetProps {
  ...
  onStatusChange: (quote: QuoteData, status: string) => void;
  onConvert: (quote: QuoteData) => void;
  ...
  return (
    <Sheet open={open} onClose={onClose} title={`Cotización ${quote.code}`} size="xl">
      ...
      <div className="flex items-start justify-between gap-3 mb-5">
        <div>
          <h2 className="text-lg font-bold text-gray-900">{customerName(quote.customer as Record<string, unknown> | null | undefined)}</h2>
          <p className="text-xs text-gray-500">
            {fmtDate(quote.quote_date || quote.quoteDate)} · Vence: {fmtDate(quote.valid_until || quote.validUntil)}
          </p>
        </div>
        <Badge variant={(STATUS_VARIANT[quote.status] || 'neutral') as BadgeVariant} className="shrink-0">
      ...
      <div className="space-y-4">
            ...
            <div className="border border-gray-200 rounded-lg overflow-hidden">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-gray-100 bg-white">
                  {quote.details.map((d: QuoteDetail, i: number) => (
                    <tr key={i} className="hover:bg-gray-50">
                      ...
                      <td className="px-3 py-2 text-right text-gray-600">{parseFloat(String(d.quantity))}</td>
                      <td className="px-3 py-2 text-right text-gray-600">{fmtUSD(d.unit_price)}</td>
                      <td className="px-3 py-2 text-right text-gray-500">
                        {parseFloat(String(d.discount_percentage || 0)) > 0 ? `${parseFloat(String(d.discount_percentage))}%` : '—'}
                      </td>
        ...
        <section className="bg-gray-50 rounded-lg p-4 border border-gray-200 space-y-1 text-sm">
          {parseFloat(String(quote.discount_amount || 0)) > 0 && (
            <div className="flex justify-between text-gray-500">
              <span>Descuento</span><span>- {fmtUSD(quote.discount_amount || 0)}</span>
            </div>
          )}
          {parseFloat(String(quote.tax_amount || 0)) > 0 && (
            <div className="flex justify-between text-gray-500">
              <span>IVA ({parseFloat(String(quote.tax_percentage || 0))}%)</span>
              <span>{fmtUSD(quote.tax_amount || 0)}</span>
            </div>
          )}

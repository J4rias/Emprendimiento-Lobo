...
const CustomerSearch: React.FC<CustomerSearchProps> = ({ isOpen, onClose, onSelect, validateCredit = false, saleAmount = 0, exchangeRates = [] }) => {
  const [customers, setCustomers] = useState<Customer[]>([]);
  const [loading, setLoading] = useState<boolean>(false);
  const [search, setSearch] = useState<string>('');
  const [error, setError] = useState<string | null>(null);
  const [creditValidation, setCreditValidation] = useState<string | null>(null);
  ...
  const getCustomerDocumentLabel = (customer: Customer): string => {

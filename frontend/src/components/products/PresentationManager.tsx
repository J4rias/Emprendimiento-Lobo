...
interface PresentationManagerProps {
  ...
  productUnitSize?: string | number;
  productUnitMeasure?: string;
}

const PresentationManager: React.FC<PresentationManagerProps> = ({ presentations = [], onChange, readonly = false, packagingTypes = [], presentationTypes = [], productUnitSize = '', productUnitMeasure = 'UND' }) => {
  const [localPresentations, setLocalPresentations] = useState<Presentation[]>([]);
  ...
  const updatePresentation = (index: number, field: string, value: string | number | boolean): void => {
  ...
  const getDefaultPresentation = (): Presentation | undefined => {

...
/** Icono de ordenacion para el encabezado */
function SortIcon({ col, sortBy, sortDir }: { col: Column; sortBy?: string; sortDir?: string }) {
...
export function Table<T extends Record<string, any> = Record<string, unknown>>({
  ...
  function handleSort(col: Column<T>): void {
  ...
  function getAriaSortFor(col: Column<T>): 'ascending' | 'descending' | undefined {
    const field = col.sortKey ?? (typeof col.accessor === 'string' ? col.accessor : null)
              ...
              return (
                ...
                >
                  <span className="inline-flex items-center gap-1">
                    {col.header}
                    <SortIcon col={col as Column} sortBy={sortBy} sortDir={sortDir} />
                  </span>
                </th>
              )
            })}
